...
st.markdown("""
    ...
    .stTextInput input, .stDateInput input, .stTextArea textarea {border-radius: 6px;}
    .st-bj {font-size: 18px;}
    .st-dp {
...
def summary_page():
    ...
    st.markdown("## 📊 Submission Details Overview")
    st.write("Thank you for submitting your daily exam operations schedule. Below is a detailed summary of your entry:")

    submitted_data = st.session_state.get("submitted_data", {})
    df = pd.DataFrame(list(submitted_data.items()), columns=["Field", "Your Entry"])

    st.write("### 🗒️ Detailed Table View")
    st.dataframe(df, use_container_width=True)

    st.markdown("---")
    st.markdown("✅ **Next Steps:** You can submit another entry or close the page. Your data is recorded successfully.")
